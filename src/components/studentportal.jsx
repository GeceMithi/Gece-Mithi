...
// === IMPORT COMPONENTS ===
import BatchSection from './card/BatchSection';
...
const App = () => {
    ...
    const handleImageChange = async (e) => { const file = e.target.files[0]; if (!file) return; if (file.size > 5 * 1024 * 1024) { showToast("File is too large (Max 1MB)", "error"); return; } if (!file.type.startsWith('image/')) { showToast("File must be an image.", "error"); return; } setLoading(true); const data = new FormData(); data.append("file", file); data.append("upload_preset", UPLOAD_PRESET); data.append("cloud_name", CLOUD_NAME); try { const res = await fetch(`https://api.cloudinary.com/v1_1/${CLOUD_NAME}/image/upload`, { method: "POST", body: data }); const uploadedImage = await res.json(); if(uploadedImage.secure_url) { setStudentForm(prev => ({ ...prev, profileImage: uploadedImage.secure_url })); showToast("Image uploaded successfully!", "success"); } else { console.error("Cloudinary Error:", uploadedImage); throw new Error("Cloudinary upload failed"); } } catch (error) { console.error("Upload Error:", error); showToast("Upload Failed.", "error"); } finally { setLoading(false); } };
    const saveProfile = async (e) => { e.preventDefault(); if (!user) return; setLoading(true); const docId = userData?.cnic || user.email.split('@')[0]; try { const docRef = doc(db, "students", docId); await setDoc(docRef, { personalInfo: studentForm }, { merge: true }); if (studentForm.email && studentForm.email !== user.email && studentForm.email.includes('@')) { try { await updateEmail(user, studentForm.email); showToast("Profile & Email Updated!", "success"); } catch(e) { console.error(e); showToast("Profile Saved, but Login Email update failed (Need recent login)."); } } else { showToast("Profile Saved Successfully!", "success"); } setUserData(prev => ({ ...prev, personalInfo: studentForm })); setIsEditing(false); } catch (error) { showToast("Save failed: " + error.message, "error"); } finally { setLoading(false); } };
    const handleCancelEdit = () => { setIsEditing(false); if(userData?.personalInfo) { setStudentForm(userData.personalInfo); } };
    const generateChallan = async (part, batch, status) => { if (!user) return; if (!studentForm.fullName || !studentForm.fatherName || !studentForm.surname) { showToast("Please complete Profile Details first.", "error"); setActiveTab('profile'); setIsEditing(true); return; } setLoading(true); const docId = userData?.cnic || user.email.split('@')[0]; const existingChallan = userData?.challans?.find(c => c.part === part); if (existingChallan) { setLoading(false); showToast(`Challan for Part-${part} already exists.`, "error"); return; } const currentCount = userData?.challans ? userData.challans.length : 0; const nextSequence = currentCount + 1; const challanNo = `CH-${batch}-${String(nextSequence).padStart(2, '0')}`; let fee = status === 'Hosteller' ? 1300 : 900; const newChallan = { id: Date.now(), challanNo, part, batch, statusType: status, amount: fee, status: 'Not Paid', generatedDate: Date.now(), hasDownloaded: false, paymentDetails: null, receiptImageUrl: null }; try { const docRef = doc(db, "students", docId); await updateDoc(docRef, { challans: arrayUnion(newChallan) }); setUserData(prev => ({ ...prev, challans: [newChallan, ...(prev.challans || [])] })); showToast("Challan Generated!", "success"); setActiveTab('dashboard'); } catch (error) { console.error(error); } finally { setLoading(false); } };
    const handleDownloadPdf = async (challan) => { if(!studentForm.fullName) { showToast("Error: Profile incomplete.", "error"); return; } generateSpecificChallanPDF(studentForm.fullName, studentForm.fatherName, challan.part, challan.batch, challan.challanNo, challan.amount, challan.statusType); if (!challan.hasDownloaded) { const docId = userData?.cnic || user.email.split('@')[0]; const updatedChallans = userData.challans.map(ch => { if (ch.id === challan.id) return { ...ch, hasDownloaded: true }; return ch; }); try { await updateDoc(doc(db, "students", docId), { challans: updatedChallans }); setUserData(prev => ({ ...prev, challans: updatedChallans })); } catch (error) { console.error(error); } } };
    const generateAdminPDF = () => { const doc = new jsPDF(); doc.setFontSize(18); let dataToPrint = allPaidStudents; let title = "Paid Challan Report - All"; if (adminFilterPart !== 'All') { dataToPrint = allPaidStudents.filter(st => st.part === adminFilterPart); title = `Paid Challan Report - Part ${adminFilterPart}`; } if (dataToPrint.length === 0) { showToast(`No records for Part ${adminFilterPart}`, "error"); return; } doc.text(`University of Sindh - ${title}`, 14, 22); const tableColumn = ["CNIC", "Name", "Father Name", "Part", "Batch", "Amount", "Date"]; const tableRows = dataToPrint.map(st => [st.studentCnic, st.name, st.fname, st.part, st.batch, st.amount, st.date]); autoTable(doc, { head: [tableColumn], body: tableRows, startY: 30 }); doc.save(`Paid_Students_${adminFilterPart}.pdf`); };
    ...
    const handleNoticePDFChange = async (e) => {
        ...
        if (file) {
            ...
            if (file.size > 5 * 1024 * 1024) { 
                showToast("PDF file size must be less than 1MB", "error");
                return;
            }
                                            ...
                                            <div className="border-2 border-[#ffd200] rounded-lg p-4">
                                                ...
                                                >
                                                    ...
                                                    <span className="text-sm text-gray-600">
                                                        {noticePDF ? noticePDF.name : 'Click to upload PDF (Max 1MB)'}
                                                    </span>
                                                </label>
                                                {noticePDF && (
